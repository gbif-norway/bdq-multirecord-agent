...
@pytest.mark.asyncio
async def test_run_tests_on_dataset_api_failure_handling(occurrence_df):
    """Test that API failures are properly handled and logged, and the loop continues"""
    svc = BDQAPIService()
    
    # Mock both requests.get and requests.post
    with patch('requests.get') as mock_get, patch('requests.post') as mock_post:
        # Mock the tests endpoint response
        mock_tests_response = _FakeResponse([
            {
                "id": "VALIDATION_COUNTRYCODE_VALID",
                "guid": "test-guid-1",
                "type": "Validation",
                "className": "TestClass",
        ...
        # Run the tests - should handle the failure gracefully
        results = await svc.run_tests_on_dataset(occurrence_df, core_type="occurrence")
        
        # Should return empty DataFrame since the batch call failed
        assert len(results) == 0, "Should return empty DataFrame when batch API fails"
        
        # Verify that requests.get was called once (to get tests)
        assert mock_get.call_count == 1, "Should have called requests.get once"
        
        # Verify that requests.post was called (for batch endpoint)
        assert mock_post.call_count > 0, "Should have called requests.post at least once"
        
        # Verify the batch call was made with correct parameters
        batch_call_args = mock_post.call_args_list[0]
        assert batch_call_args[1]['json'] is not None, "Batch call should have JSON payload"
        assert len(batch_call_args[1]['json']) > 0, "Batch call should have test requests"
